<template>
    <v-app>
        <v-main>
           ...
           position: absolute>
           <v-btn v-if="btnalert==1" class="warning" @click="alert=!alert"><h2>外借人和借用部门不能为空</h2></v-btn>
           <v-btn v-else-if="btnalert==2" class="success" @click="alert=!alert"><h2>外借成功</h2></v-btn>
 ...
 <v-row justify="space-around">
  ...
  <h1>待外借设备表</h1>  
    ...
    >
         ...
         <v-card-title class="red darken-3">
    <v-text-field v-model="waijieren" label="借用人" />
    <v-spacer/>
     <v-text-field v-model="waijiebm" label="借用部门" />
    <v-spacer/>
     <v-text-field v-model="waijieliyou" label="借用理由" />
     <v-spacer/>
       <v-btn color="black" @click="QRPD"><h2>确认外借</h2></v-btn>
         </v-card-title>
            </v-card>
                </v-col>
            </v-row>
...
export default {
    ...
    methods:{
      QRPD(){
        if (this.selected.length<1||this.waijieren==""||this.waijiebm==""){
           this.btnalert=1;
           this.alert=true;
        }
        else{ 
